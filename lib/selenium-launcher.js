...
(function () {
    ...
    function init(options) {
        ...
        seleniumServerFilename = 'selenium-server-standalone-' + seleniumServerVersion + '.jar',
            seleniumServerUrl = 'http://selenium-release.storage.googleapis.com/' + seleniumServerVersion.substr(0, seleniumServerVersion.lastIndexOf('.')) + '/' + seleniumServerFilename,
            ...
            chromeDriverUrl = 'http://chromedriver.storage.googleapis.com/' + chromeDriverVersion + '/' + chromeDriverFilename,
        ...
        fs.stat(outfile, function (er) {
            ...
            hashFile(outfile, 'sha1', function (er, actualSha) {
                ...
                if (actualSha !== expectedSha) {
                    console.log("Downloading reason: SHA configured in rtd.config.js does not match '%s'", actualSha);
                    return real();
                }
                cb();
            });
